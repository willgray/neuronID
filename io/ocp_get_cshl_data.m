function img = ocp_get_cshl_data(serverLocation, token, resolution, outputDir, xArgs, yArgs, zArgs)
...
for ii = 1:size(img.data,3)
    ...
    imwrite(squeeze(img.data(:,:,ii,1:3)), filename)
end

%% Save nii
filename = fullfile(dirname, sprintf('%s_x%d-%d_y%d-%d_z%d-%d_r%d.nii',...
    token,xArgs(1),xArgs(2),yArgs(1),yArgs(2),zArgs(1),zArgs(1)+ ii - 1,resolution));
nii = make_nii(img.data(:,:,:,1:3), [1 1 1], [0 0 0], 128);
